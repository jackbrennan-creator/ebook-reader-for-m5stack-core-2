...
class ebook
{
  ...
  private:
    ...
    void display_current_page()
    {
      ...
      display->setCursor(display->width() / 2, display->height()); 
    ...
    void display_word(int index)
    {
      ...
      for (int pt = start; pt <= finish; pt = pt + 1) display->print(line[pt]);
    }
    ...
    {
      const char min_ascii_ch = ' ';
      const char max_ascii_ch = '~';
      if (min_ascii_ch <= ch && ch <= max_ascii_ch)
      {
